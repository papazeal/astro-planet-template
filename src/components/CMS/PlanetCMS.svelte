...
<form method="POST" onsubmit={submit}>
  <div class="shadow-md">
    <div class="py-4 px-7 flex gap-8">
      <div class="flex gap-2">
        <svg
          ...
          /></svg
        >
        <a href={website}>{website.replace(/(^\w+:|^)\/\//, "")}</a>
      </div>
      ...
      <div class="ml-auto">
        <div
          class="fixed flex md:relative md:p-0 md:border-0 md:shadow-none bottom-0 left-0 w-full p-4 shadow-lg border-t border-slate-300 gap-4 z-20"
        >
          <a
            href="/signout"
            ...
            {#each group.fields as field}
              <div class="">
                ...
                <input
                  ...
                  class="border border-gray-400 px-3 py-1.5 rounded min-w-100 max-w-full"
                />
              </div>
            {/each}
          </div>
